function resolveApiBase() {
    const override = (window.NEUROPAY_API_BASE || "").trim();
    if (override) {
...
function setConnectionState(mode, label) {
    ...
    if (mode === "live") {
        connectionStatus.classList.add("bg-emerald-400");
    } else if (mode === "connecting") {
        connectionStatus.classList.add("bg-amber-400");
    } else {
        connectionStatus.classList.add("bg-rose-400");
    }
}
...
function handleSocketPayload(payload) {
    ...
    if (type === "report_ready") {
        if (payload.report) {
            reportContent.innerHTML = formatMarkdown(payload.report);
        }
...
async function startResearch() {
    ...
    try {
        ...
        if (!liveEventsSeen) {
            ...
            if (data.report) {
                reportContent.innerHTML = formatMarkdown(data.report);
            }
...
function processResultsFallback(data) {
    ...
    reportContent.innerHTML = formatMarkdown(data.report || "# Empty report");
    renderMarginTable(data.summary?.margin_analysis?.rows || []);
}
...
function appendTransactionRow(tx, prepend = true) {
    ...
    row.className = "glass p-4 rounded-2xl flex justify-between items-center gap-3 animate-fade-in border-l-4 border-cyan-400";
    row.innerHTML = `
        <div class="flex flex-col min-w-0">
            <span class="text-xs text-cyan-300 font-bold uppercase tracking-wide">${escapeHtml(tx.domain || "GENERAL")}</span>
            <span class="text-sm text-slate-100 truncate">${escapeHtml(tx.question || "")}</span>
        </div>
        <div class="flex items-center gap-3 shrink-0">
            <span class="text-cyan-200 font-mono text-sm">$${amount}</span>
            <a href="${arcUrl}" target="_blank" rel="noreferrer"
               class="text-xs bg-slate-900 hover:bg-slate-800 text-slate-300 px-2 py-1 rounded border border-slate-600 transition-colors">
...
function appendErrorRow(domain, error) {
    const row = document.createElement("div");
    row.className = "glass p-3 rounded-2xl border-l-4 border-rose-400 text-rose-200 text-sm";
...
function renderMarginTable(rows) {
    ...
    for (const row of rows) {
        ...
        tr.className = `border-b border-slate-800/80 ${isCircle ? "bg-cyan-500/10 text-white" : ""}`;
        ...
        tr.innerHTML = `
            ...
            <td class="py-3 ${isCircle ? "text-emerald-300 font-bold" : "text-slate-300"}">${relative}</td>
        `;
...
function formatMarkdown(text) {
    ...
    return escapeHtml(text)
        .replace(/^### (.*$)/gim, '<h3 class="text-xl font-semibold text-cyan-200 mt-5 mb-2">$1</h3>')
        .replace(/^## (.*$)/gim, '<h2 class="text-2xl font-bold text-white mt-6 mb-3">$1</h2>')
        .replace(/^# (.*$)/gim, '<h1 class="text-3xl font-black text-white mb-4">$1</h1>')
